...
export default function TokenList({ tokens, type }: TokenListProps) {
    const [logoErrors, setLogoErrors] = useState<{ [key: string]: boolean }>({}); // State to track logo errors

    const handleError = (symbol: string) => {
        setLogoErrors((prev) => ({ ...prev, [symbol]: true })); // Update error state for specific token
    };
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                            <div className="relative w-8 h-8">
                                {!logoErrors[token.symbol] && token.logoUrl ? ( // Check for logo error
                                    <Image
                                        src={token.logoUrl}
                                        alt={token.name}
                                        fill
                                        className="rounded-full"
                                        onError={() => handleError(token.symbol)} // Handle image load error
                                    />
                                ) : (
